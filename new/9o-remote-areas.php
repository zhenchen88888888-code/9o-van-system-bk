/**
 * Code Snippet: [003] 9O Booking - remote-areas
 * 
 * 處理所有偏遠地區加價邏輯
 * 包含完整的35個多邊形座標定義
 * 
 * Code Snippets 設定:
 * - Title: [003] 9O-remote-areas
...
class NineoRemoteAreas {
     ...
     * 偏遠地區定義（完整35個多邊形）
     */
    private static function get_remote_areas() {
        return [
            // +300 區域（3個多邊形）
            [
                'name' => '加價300區域',
    ...
    public static function check_remote_area_surcharge($address) {
        ...
        $remote_areas = self::get_remote_areas();
    ...
    private static function get_address_coordinates($address) {
        // 檢查是否有核心配置
        if (!class_exists('NineoConfig')) {
            error_log('NineoConfig 類別不存在，請確保核心模組已載入');
            return false;
        }
...
add_action('init', function() {
    error_log('9O Remote Areas Module loaded - 35 polygons registered');
});